...
console.log('bolum basligi:', (await sayfa.locator('.gorev-karti h1').textContent()) ?? '');
...
console.log('rapor:', ((await sayfa.locator('.rapor').first().textContent().catch(() => '')) ?? '').trim().slice(0, 120));
const dugmeler = await sayfa.locator('button').allTextContents();
console.log('dugmeler:', dugmeler.filter(Boolean).slice(0, 20).join(' | '));
